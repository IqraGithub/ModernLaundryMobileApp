...
const ProductItem = React.memo(
  ({ product, index, selectedEmirate }) => {
    ...
    let f = useSelector((state) => {
      return state.filteredData.filteredPricing[product.id];
    });
    ...
    useEffect(() => {
      async function getEmirateId() {
        try {
          const emirate = emirates.data.find(
    ...
    useEffect(() => {
      if (emirateId) {
        let filteredPricingData = product.pricing?.filter(
          (obj) => obj.deliveryType === deliveryType && obj.emirate_id == emirateId
        );
